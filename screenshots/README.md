...
The `gallery` target fills `screenshots/gallery/` with the frames [GALLERY.md](../GALLERY.md) shows and the README does not: the radar in its fixed themes and its satellite and condition layers, the sky in more traditions, the weather in a short window, the moon's month grid, and a walking route. The README's own frames stay in this directory; a new state belongs in the gallery target and on the gallery page, not in the README, unless it earns a place there.
